...
class User extends Base {
	...
	/**
	 * 读取成员信息
	 * @param {Object} userid - 成员UserID，企业内必须唯一，不区分大小写，长度为1~64个字节
	...
	async getUser(userid) {
		...
		const res = await this.curl(url, 'GET', {
			userid
		}, 'agent');
		return res;
	}
	...
	async deleteUser(userid) {
		...
		const res = await this.curl(`https://qyapi.weixin.qq.com/cgi-bin/user/delete`, 'GET', {
			userid
		}, 'concat');
		return res;
	}
	...
	async batchDeleteUser(useridlist) {
		...
		const res = await this.curl('https://qyapi.weixin.qq.com/cgi-bin/user/batchdelete', 'POST', {
			useridlist
		}, 'concat');
		return res;
	}
	...
	async getDepartmentUsers(department_id) {
		...
		const res = await this.curl(`https://qyapi.weixin.qq.com/cgi-bin/user/simplelist`, 'GET', {
			department_id
		}, 'agent');
		return res;
	}
	...
	async getDepartmentUserDetails(department_id) {
		...
		const res = await this.curl(`https://qyapi.weixin.qq.com/cgi-bin/user/list`, 'GET', {
			department_id
		}, 'agent');
		return res;
	}
	...
	async convertToOpenid(userid) {
		...
		const res = await this.curl('https://qyapi.weixin.qq.com/cgi-bin/user/convert_to_openid', 'POST', {
			userid
		}, 'agent');
		return res;
	}
	...
	async convertToUserid(openid) {
		...
		const res = await this.curl('https://qyapi.weixin.qq.com/cgi-bin/user/convert_to_userid', 'POST', {
			openid
		}, 'agent');
		return res;
	}
	...
	async authSucc(userid) {
		...
		const res = await this.curl(`https://qyapi.weixin.qq.com/cgi-bin/user/authsucc`, 'GET', {
			userid
		}, 'agent');
		return res;
	}
	...
	async inviteUser(params) {
		...
		const res = await this.curl('https://qyapi.weixin.qq.com/cgi-bin/batch/invite', 'POST', params, 'agent');
		return res;
	}
	...
	async getUseridByMobile(mobile) {
		...
		const res = await this.curl('https://qyapi.weixin.qq.com/cgi-bin/user/getuserid', 'POST', params, 'agent');
		return res;
	}
	...
	async getUseridByEmail(email, emailType) {
		...
		const res = await this.curl('https://qyapi.weixin.qq.com/cgi-bin/user/get_userid_by_email', 'POST', params, 'agent');
		return res;
	}
